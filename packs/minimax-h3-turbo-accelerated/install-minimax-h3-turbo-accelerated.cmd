...
echo. & echo Done. Restart SimpliGen. & echo. & echo IMPORTANT: Three custom nodes must be installed first: & echo - ComfyUI-KJNodes (SageAttention) & echo - ComfyUI-Spectrum-MiniMax-H3 (Spectrum) & echo - ComfyUI-SolAttn_triton (Sol-Attn) & echo. & echo SageAttention now comes from SimpliGen itself (Settings ^> Advanced ^> & echo faster attention), which injects the KJNodes patch node - so that node & echo must be present even though the workflow no longer names it. Without & echo faster attention these presets still run, just slower. & echo. & pause
exit /b 0
:error
