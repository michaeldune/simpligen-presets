...
def make_install(data, slug):
    ...
    ps1 = (
        ...
        '        $civitaiToken = Read-Host "  Civitai token"\n'
        '    }\n'
        '    Write-Host ""\n'
        '    foreach ($m in $models) {\n'
        ...
        '        try {\n'
        '            if ($isHF) {\n'
        '                Invoke-WebRequest -Uri $m.url -OutFile $destPath -Headers @{ Authorization = "Bearer $hfToken" }\n'
        '            } elseif ($isCivitai) {\n'
        '                $sep = if ($m.url -like \'*?*\') { \'&\' } else { \'?\' }\n'
        '                $u = $m.url + $sep + \'token=\' + $civitaiToken\n'
        '                Start-BitsTransfer -Source $u -Destination $destPath\n'
        '            } else {\n'
        '                Start-BitsTransfer -Source $m.url -Destination $destPath\n'
        '            }\n'
