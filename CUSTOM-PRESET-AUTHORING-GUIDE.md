# SimpliGen Custom Local Preset Authoring Guide (v7)

> **What changed in v7 (2026-09-22)** — read these first if you knew v6:
> - **Declare every custom node, rgthree included** (§7.6). v6 said rgthree was exempt; it is not. Packs that used the Power Lora Loader without declaring it only ran for people who had it from another pack. `tests/check_declared_extensions.py` now checks this (§13).
> - **The presets folder is the `data_dir` setting**, not always `%APPDATA%` (§1).
> - **Numbers stay bare** (`{{width}}`, `{{height}}`, `{{seed}}`); §2.6 no longer tells you to quote everything (§2, §8.2).
> - New: pack-level `checksums` and how the store pins downloads (§6.1); pin Hugging Face URLs to a commit (§6.1); optional reference slots with `sgopt_` (§7.5b); `characterContract.maxCharacters` for the cast picker (§6.2); the negative prompt arrives empty (§8.2); store thumbnails show only the top strip (§11); test copies share workflow files with the store copy (§14); new failure rows (§15); Qwen-Image 2.1 and H3 lip-closure notes (§9, §10).
...
## 1. Locations
```text
Installed packs:      <data_dir>\presets\<pack>.json          (default data_dir: %APPDATA%\simpligen)
Installed workflows:  <data_dir>\presets\workflows\<id>.json   (shared across packs)
Installed thumbnails: <data_dir>\presets\previews\<id>.jpg     (shared)
Engine models root:   <data_dir>\models\  (older installs: %APPDATA%\simpligen\engine\models\)
...
```
- **Find `data_dir` before installing anything.** Users can move SimpliGen's data to another drive (Settings), and the app then reads packs only from `<data_dir>\presets`. Read it from the `settings` table of `%APPDATA%\simpligen\simpligen.db` (key `data_dir`). A pack copied into `%APPDATA%\simpligen\presets` on such a machine is silently ignored, and a restart proves nothing.
...
6. Validate every JSON with a parser. **Numbers that feed typed nodes stay bare** (`"width": {{width}}`, `"height": {{height}}`, `"seed": {{seed}}`), exactly as the official templates do: a quoted `"{{width}}"` arrives as a string and nodes such as `ComfyMathExpression` fail with `'>' not supported between 'str' and 'int'` — only inside the app, never in an engine test that substituted real numbers. Text and filenames stay quoted (`"unet_name": "{{unet}}"`). A bare placeholder is not valid JSON until substituted, so *any tool you write must handle both forms* — and must preserve which form each placeholder used. Substitute quoted `"{{x}}"` and bare `{{x}}` with **different sentinels** and invert each separately; a single blind substitution turns `"unet_name": "{{unet}}"` into `""sentinel""` on the way in, or drops the quotes on the way out. Assert after the round trip that every placeholder name survived *with the same quoting*.
...
**4.7 Custom nodes:** clone the repo into `engine\ComfyUI\custom_nodes\` (the bundled engine loads custom nodes). Some nodes happen to be present on most machines, but a preset may only rely on what it declares in `extensions[]` (§7.6). SimpliGen has no custom UI slider, so to expose a node parameter you can repurpose the `steps`/`cfg` slider (route `{{cfg}}` into the node param and hardcode the sampler cfg). See §7.3 for the two very different *kinds* of "custom node dependency" — a git clone is not always the whole story.
...
- **`minComfyuiVersion` is read per pack, not per preset.** Put it at the pack's top level; one on a preset is ignored.

**6.1 Downloads the store will serve: `checksums` and pinned URLs.** The store checksums every model entry a pack declares and refuses to publish the pack if even one cannot be pinned. You can supply the hashes yourself with a pack-level map, which is also how a Civitai file gets pinned:
```json
"checksums": {
  "https://huggingface.co/Comfy-Org/<repo>/resolve/<40-char commit>/<path>/<file>.safetensors": "<sha256>",
  "https://civitai.com/api/download/models/<versionId>": "<sha256>"
}
```
- **Pin Hugging Face URLs to a commit** (`/resolve/<sha>/...`, not `/resolve/main/...`). A repo owner can move or rename a file at any time; `main` URLs then 404 for every new user while your PC keeps working (the H3 latent upscaler moved into a subfolder on 2026-09-22 and broke H3 Refine and Two-Stage until they were repinned). Get the commit from `https://huggingface.co/api/models/<owner>/<repo>` (`sha`) and the file hash from the `X-Linked-Etag` response header.
- Civitai: take the SHA-256 from `https://civitai.com/api/v1/model-versions/<versionId>` (`files[].hashes.SHA256`) and compare it with the file on disk before shipping. The app downloads Civitai files with the user's Civitai token; if a user has none, they are asked for one.
- Small non-LFS files on Hugging Face (config.yaml, vocab.json) have no content-hash ETag; the store now hashes those itself, but prefer packs that do not need them.

**6.2 Character picker: `characterContract.maxCharacters`.** From app 1.65.0 the cast picker lets a user put several saved characters into one generation, up to the number the preset declares. Undeclared means **1**. Declare it on presets whose reference slots are interchangeable subjects, and leave it at 1 when each slot has a fixed meaning (person + outfit, first + last frame):
```json
"characterRoles": ["identity_edit"],
"characterContract": { "placeholders": ["prompt"], "inputImage": "ref_image_1", "maxCharacters": 4 }
```
Characters and uploaded reference pictures share the same slots, so `maxCharacters` should not exceed `acceptsReferenceImages.max`. Verified 2026-09-22: four characters cast on a 4-slot Qwen edit card all reached the render, each `@name` rewritten to `[image N]`.
...
`min` on `acceptsReferenceVideos` / `acceptsReferenceAudios` is honoured by the app's pre-submit gate only from the build after 1.59.2; before that only the image minimum disabled Generate, so a preset that requires a clip (the chain packs) could be dispatched without one and failed in the engine instead (see §15). No first-party preset sets a clip or sound minimum, so this only bites presets that declare one — declare it anyway, the gate reads it now.
```
...
**7.5b Optional reference slots: the `sgopt_` naming.** A loader whose node id is `sgopt_<slot>__<suffix>` (for example `sgopt_ref_image_2__load`) is optional: when the user leaves that slot empty, the app deletes the node **and** every input that pointed at it (such as an `images.image_2` key on a multi-input encoder) before queueing. One encoder can therefore serve 1..N pictures without a switch per slot. To branch the graph on whether a slot was filled, add a `PrimitiveBoolean` named `sgopt_<slot>__flag` (value `true`) and feed it into `ComfyOrNode(values.value0=false, values.value1=[flag])`: when the slot is empty the flag disappears with it and the Or returns false. Use the **lazy** `ComfySwitchNode` on that result so the unused branch never loads its model (`ComfySoftSwitchNode` exists in the source but is not registered).
...
- **Declare every custom node the workflow uses, and give each a `pinnedCommit` — `rgthree-comfy` included.** v6 of this guide exempted rgthree as a baseline node; that was wrong. The app only installs a node a preset declares, so a workflow that uses `Power Lora Loader (rgthree)` (every `simpligen_lora_1` marker) or `UnetLoaderGGUF` without declaring it runs only on machines that got the node from another pack. That is how Krea CSG Foundation failed for a user with `Node 'Load Krea-CSG Foundation (GGUF)' not found` for a month (fixed 2026-09-22, with 99 other undeclared rgthree uses across 24 packs). Run `tests/check_declared_extensions.py` (§13). If your packs are generated by a builder script, put the declaration in the builder: a rebuild otherwise drops a hand-added entry. Leaving a pin out is not "track the branch": the store refuses to serve an untrusted node from a moving reference, so at sync time it resolves the branch head itself and writes it into the served pack as `commit`. On app <= 1.59.2 readiness treated that store-written `commit` as an exact pin, so every user whose checkout sat at any other commit saw the cards stuck on "Install preset locally" — that is what took all three Singularity 1.0.0 cards down on 2026-09-10 (store wrote KJNodes `da90cca`, engines held the `3f20054` every other pack pins; it had drifted to `5710537` a day later). The next app build enforces only an author-declared `pinnedCommit` and the sync warns which nodes it had to pin itself, but an explicit pin is still what makes the pin enforceable at all.
- **A pack withdrawn from the repo and put back at the same version is never re-published.** The store keys on pack id + version; the TTS Lip-Sync pack returned at 1.0.0 on 2026-09-09 and was skipped by every sync until it was bumped to 1.0.1. Any return to master is a version bump, even when the content is byte-identical.

**7.6b No `cloud` block in a community pack.** A preset copied from an official manifest carries `"cloud": {"appGroup": "minimax-h3"}` (and often a `cloudWorkflow`). The store refuses the whole pack before assigning a version: `appGroup 'minimax-h3' must start with the pack's own prefix ... or name a family pool`, because a community pack can never share a first-party Modal app. Delete `cloud` from every preset (and `cloudWorkflow` from the media block); cloud is added on the store's side when a pack is onboarded, and the sync drops anything an author writes there (Sharmystic, 2026-09-16; cost TaoMate and FastH3 a sync).
...
- **The Enhance page adds these to the context for every tool job** (from the job payload, 1.59.2): `scale` (scale mode) or `targetResolution` (target mode); `resolution`; `outputWidth` / `outputHeight` (the exact size the picker showed); `inputWidth` / `inputHeight`; `sourceName` (input filename without extension); `tool`. Measured on 2026-09-10 with a two-node probe preset (941×1672 portrait input), values as stored in the job:

  | mode | `scale` | `targetResolution` | `resolution` | `outputWidth`×`outputHeight` | `inputWidth`×`inputHeight` |
  |---|---|---|---|---|---|
  | scale, 2x | 2 | (absent) | 1882 | 1882×3344 | 941×1672 |
  | target, 1440p | (absent) | 1440 | 1440 | 1440×2559 | 941×1672 |

  So `resolution` is the output **short side** in both modes, and a workflow that uses the other mode's key fails preflight with `missing render settings`.
- **The app overwrites `SaveImage`/`SaveVideo` `filename_prefix`** with its own `<project>/<pack><preset>/<source>_upscaled` path (generate presets get their own scheme too). Do not encode anything you need in the prefix; it never survives.
- **`{{resolution}}` is set in both modes:** the Enhance page sends it as the output short side; the local renderer also has a fallback (1080 below 4x, 2160 at 4x and up) that only fires when no `resolution` arrived, which the Enhance page never leaves empty. It exists for SeedVR2-style nodes that take an absolute short side. In *target* mode the page sends `resolution` itself alongside `targetResolution`. Summary: scale mode gives `{{scale}}` + `{{resolution}}`; target mode gives `{{targetResolution}}` + `{{resolution}}`; `{{outputWidth}}`/`{{outputHeight}}` are the safest choice when a node wants exact pixels.
...
- **A placeholder the context lacks is rejected before the engine sees it.** After substitution the app scans for any remaining `{{name}}` (optional reference slots excepted) and fails the job with `Could not prepare this preset for generation: missing render settings {{a}}, {{b}}` — listing *every* unfilled key, and logging them under `Unfilled workflow placeholders` in the session log with the media-block keys it had. That message is your checklist: for each key named, add the block field, use the computed value, or drop the placeholder.
...
- **The negative prompt arrives empty unless the user types one.** The media block's `negativePrompt` is not applied as a default (confirmed through the agent API on 2026-09-22: a card with a 180-character `negativePrompt` rendered with `negative_prompt: ""`). If a preset depends on a negative, build it into the workflow and append the user's: `StringConcatenate(string_a="<built-in negative>", string_b="{{negative_prompt}}", delimiter=", ")` into the encoder.
- **A node with `validate_inputs` cannot take a link on the inputs that method reads.** Validation runs before execution, a linked input is `None` there, and `float(None)` kills the graph with `Exception when validating inner node`. Read the node class before wiring a computed value into a widget; if it validates, pass a substituted `{{placeholder}}` instead.
...
| MiniMax H3 Video Refine (V2V) | euler, PDD scheduler denoise 0.25 | 8 grid (2 run) | 1 | Source clip via `{{ref_video_1}}`, audio from GetVideoComponents (the soundtrack slot is empty from the agent API), H3 VAE encode, LBH 3D latent upscale to the tier, MMH3SplitUpscale 73/22 windows, source audio muxed back |
...
| Qwen-Image 2.1 (text to image / edit) | seeds_2 / sgm_uniform, with `APG` (eta 1, norm 10, momentum 0.3) → `FreSca` (1 / 2 / cutoff 8) on the model | 20 | 3 | Comfy's template ships CFG 1, which garbles lettering; CFG 3 with APG + FreSca and a negative prompt fixed signs and menus without the oversaturation plain euler at CFG 3–4 gives (2026-09-22, 2 prompts × 3 seeds + edit tests). About 2.4× the time of CFG 1. Outputs RGBA: drop alpha with `SplitImageWithAlpha` before video |
...
- **Video (MiniMax H3 style):** prompts describe a timeline, not a single frame — shots, camera motion, dialogue/speaker tags, and separate `overall_soundscape`/`non_diegetic_music` fields if the model supports audio. Treat it as writing a short shot list, not an image prompt. **Claim the time after a spoken line**, or H3 fills it with invented words: `From 0 to 3 seconds she says "...". At 3 seconds her lips close. From 3 seconds to the end of the shot she is silent, lips closed, and <an observable action>.` Meta-instructions ("don't pad", "keep it short") have no effect.
...
- Run `uv run python tests/check_declared_extensions.py` with SimpliGen's engine running: it asks the engine which package every workflow `class_type` comes from and lists any custom-node package a preset block uses but does not declare in `extensions[]`. It must print `gaps: 0`.
...
- After install: restart SimpliGen, check the newest `session-*.log` for "Loaded N preset packs" and no "Failed to load preset pack". Run a test generation **through the app** (not only by posting the workflow to the engine: the app's substitution, pruning and defaults are what break things) and confirm real output (an image file, or for video, a playable file with the requested duration — check via `ffprobe`, since duration sliders often get frame-quantized and don't land on the exact requested number of seconds).
...
- **Store-installed and hand-installed copies of the same pack coexist.** A catalogue install lands under id `community--<slug>-pack`; the repo's own installer lands under `<slug>-pack`. Different ids, so the app treats them as unrelated and shows both, with the hand-installed one frozen at whatever version it was. After moving a pack into a catalogue, remove the local copy or you accumulate stale duplicates — and be aware the local copy may be the *newer* one until the catalogue ingests your release. When you remove the local copy by hand, delete **only** its manifest and its rows in `preset_packs` and `presets` (app closed, DB backed up): the store copy references the same `workflows\*.json` and `previews\*.jpg` **by filename**, so deleting the local pack's files breaks the store copy. Never hand-place a manifest under the store's `community--<slug>-pack.json` filename with the bare id inside: the app registers it as a `bundled` pack and re-creates its DB row on every start until the file is overwritten by a real store install.
- **A hand-installed test copy shares its workflow files with the store copy.** If your new version adds a placeholder (say `{{unlock}}`), copying its workflows into the presets folder also changes what the older store copy runs, and the store copy's manifest does not supply the new key. Test with the hand-installed copy only until the store syncs the new version. A new manifest file is only picked up after an app restart.
...
- **`Required input is missing: image` on `chain_prev_count` / `chain_tail` (or any node fed by an `sgopt_ref_video_*` loader), preceded by `No video uploaded (skipping video_file)`:** the job was submitted with no clip in the reference-video slot, so the app pruned the loader and the nodes downstream lost their input. User error (clip goes in Reference Video 1), surfaced as "the preset may need updating" because app <= 1.59.2 did not gate Generate on `acceptsReferenceVideos.min`. Fixed in the next build.
- **Every card of a pack stuck on "Install preset locally" with all weights present, and the engine log shows `Extension 'x' is at <a>, pack pins <b>`:** an extension pin mismatch, not a model. On <= 1.59.2 this includes a `commit` the store wrote for a node you left unpinned (§7.6). Declare `pinnedCommit` and ship a version bump.
- **A pack you pushed never appears in the store while later packs do:** same id + version as something the store already ingested (typically a withdraw-and-return). Bump the version.
- **`Node '<title>' not found` / `Cannot execute because a node is missing the class_type property`, while the pack works on your PC:** a custom node the workflow uses is not declared in `extensions[]`, so the app never installed it for that user (§7.6). Your machine has it from another pack.
- **A card that worked stops downloading for new users with a 404:** the model's Hugging Face URL used `/resolve/main/` and the owner moved or renamed the file. Repin to a commit (§6.1) and bump the version.
- **Output ignores the negative prompt you set in the manifest:** it was never sent (§8.2). Build it into the workflow.
- **The store copy of a pack breaks right after you install a test copy:** shared workflow files (§14).
