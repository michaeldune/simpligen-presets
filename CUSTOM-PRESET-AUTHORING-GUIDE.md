...
- **`{{resolution}}` is a third, derived key (traced in app 1.59.2):** in *scale* mode the local renderer sets `resolution` to 1080 for scales below 4 and 2160 for 4x and up, unless your media block already defines `resolution`. It exists for SeedVR2-style nodes that take an absolute short side. In *target* mode nothing synthesizes it: use `{{targetResolution}}` there, or set `resolution` in the block yourself. Summary: scale mode gives `{{scale}}` + `{{resolution}}`; target mode gives `{{targetResolution}}` only.
